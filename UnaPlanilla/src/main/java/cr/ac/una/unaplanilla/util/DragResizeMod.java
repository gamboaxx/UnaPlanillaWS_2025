...
public class DragResizeMod {
    ...
    private DragResizeMod(Node node, OnDragResizeEventListener listener) {
        ...
        if(this.node instanceof Region){
            this.MIN_W = ((Region)this.node).getMinWidth();
            this.MIN_H = ((Region)this.node).getMinHeight();
        }
        if (listener != null)
    ...
    /***
     * Devulve el estado del mouse según posición en la pantalla
     * @param event
     * @return 
     */
    ...
    /***
     * Devuelve el tipo de cursor según la posicición en la pantalla
     * @param state
     * @return 
     */
